...
class DownloadYTAudio extends EventEmitter {
   ...
   */
  async download (videoId, fileName = 'out.mp3') {
    ...
    const videoSetting = await once(videoStream, 'info')
    this.emit('video-info', videoSetting)

    // ffmpeg.setFfmpegPath(path.join(__dirname, '../ffmpeg/bin/ffmpeg')) // TODO
